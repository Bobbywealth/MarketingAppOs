...
`;
  ...
  app.get("/api/google/calendar/auth", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      console.error(error);
      res.status(500).json({ message: error.message || "Failed to get auth URL" });
    }
  });
  ...
  app.get("/api/google/callback", async (req: Request, res: Response) => {
    try {
      ...
      res.redirect('/company-calendar?connected=true');
    } catch (error: any) {
      console.error(error);
      res.redirect('/company-calendar?error=auth_failed');
    }
  });
  ...
  app.post("/api/google/calendar/sync", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      if (!userRecord?.googleAccessToken) {
        return res.status(400).json({ message: "Google Calendar not connected" });
      }

      const { GoogleCalendarService } = await import("./googleCalendarService");
      const googleService = new GoogleCalendarService();
      await googleService.setCredentials(userRecord.googleAccessToken, userRecord.googleRefreshToken || undefined);
      
      // Get all calendar events
      const events = await storage.getCalendarEvents();
      
      // Sync events to Google Calendar
      const syncedEvents = await googleService.syncEvents(events);
      
      // Update local events with Google event IDs
      for (const synced of syncedEvents) {
        await storage.updateCalendarEvent(synced.localId, {
          googleEventId: synced.googleId,
        });
      }
      ...
      res.json({ 
        ...
        message: `Successfully synced ${syncedEvents.length} events to Google Calendar` 
      });
    } catch (error: any) {
      console.error(error);
