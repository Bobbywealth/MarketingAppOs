...
export interface IStorage {
  ...
  getOnboardingTasksByClient(clientId: string): Promise<OnboardingTask[]>;
  createOnboardingTask(task: InsertOnboardingTask): Promise<OnboardingTask>;
  updateOnboardingTask(id: string, data: Partial<InsertOnboardingTask>): Promise<OnboardingTask>;
  toggleOnboardingTaskCompletion(id: string): Promise<OnboardingTask>;
  deleteOnboardingTask(id: string): Promise<void>;
  createDefaultOnboardingTasks(clientId: string): Promise<OnboardingTask[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getOnboardingTasksByClient(clientId: string): Promise<OnboardingTask[]> {
    return await db
      .select()
      .from(onboardingTasks)
      .where(eq(onboardingTasks.clientId, clientId))
      .orderBy(onboardingTasks.dueDay);
  }
  ...
  async updateOnboardingTask(id: string, data: Partial<InsertOnboardingTask>): Promise<OnboardingTask> {
    const [task] = await db
  ...
  async toggleOnboardingTaskCompletion(id: string): Promise<OnboardingTask> {
  ...
  async createDefaultOnboardingTasks(clientId: string): Promise<OnboardingTask[]> {
    const defaultTasks = [
      {
        clientId,
      ...
      },
      {
        ...
        description: "Set up initial strategy call to understand goals",
        dueDay: 2,
        category: "setup",
      },
      {
        ...
        description: "Get logos, brand guidelines, and existing materials",
        dueDay: 3,
        category: "setup",
      },
      {
        ...
        description: "Have client fill out detailed questionnaire",
        dueDay: 3,
        category: "setup",
      },
      {
        clientId,
        title: "Set Up Social Media Access",
        description: "Get admin/editor access to all social platforms",
        dueDay: 5,
        category: "setup",
      },
      {
        ...
        description: "Create 30-day content calendar and posting schedule",
        dueDay: 7,
        category: "content",
      },
      {
        clientId,
        title: "First Content Batch Created",
        description: "Create and get approval for first 2 weeks of content",
        dueDay: 10,
        category: "content",
      },
      {
        ...
        description: "Configure Google Analytics, Facebook Pixel, etc.",
        dueDay: 7,
        category: "setup",
      },
      {
        ...
        description: "Walk client through dashboard and approval process",
        dueDay: 14,
        category: "training",
      },
      {
        ...
        description: "Launch initial marketing campaign",
        dueDay: 14,
        category: "content",
      },
      {
      ...
      },
      {
        ...
        description: "Review first 2 weeks performance and adjust strategy",
        dueDay: 21,
        category: "review",
      },
      {
        ...
        description: "Complete 30-day review and transition to ongoing service",
        dueDay: 30,
        category: "review",
      },
    ];

    const createdTasks = await db.insert(onboardingTasks).values(defaultTasks).returning();
    return createdTasks;
  }
