...
const getFollowUpBadge = (
  ...
  const fallbackDate = updatedAt
    ? addDays(new Date(updatedAt), 1)
    : addDays(new Date(), 1);
  const dueDate = nextFollowUp ? new Date(nextFollowUp) : fallbackDate;

  let label = `Due ${format(dueDate, "MMM d")}`;
  ...
  return { dueDate, label, badgeClass };
};
const getScoreBadge = (score: string | null) => {
  ...
  return (
    <Badge
      variant="outline"
      className={`ml-2 text-[10px] px-1.5 py-0 h-4 border ${colors[score as keyof typeof colors] || ""}`}
    >
            ...
            {leads.map((lead) => (
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <Badge
                          variant="outline"
                          className="text-[10px] h-4 px-1 py-0 font-normal bg-slate-50 text-slate-500 border-slate-200"
                        >
                ...
                <td className="p-4">
                  <Badge
                    variant="outline"
                    className="text-xs font-normal bg-violet-50 text-violet-700 border-violet-200"
                  >
                    {getSourceLabel(lead.source)}
                  </Badge>
                </td>
                ...
                <td className="p-4">
                  {(() => {
                    const followUp = getFollowUpBadge(
                      lead.nextFollowUp,
                      lead.updatedAt,
                    );
                    return (
                      <>
                        <Badge
                          variant="outline"
                          className={`text-xs font-medium ${followUp.badgeClass}`}
                        >
                          {followUp.label}
                        </Badge>
                ...
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    <Avatar className="h-7 w-7 border border-white shadow-sm">
                      <AvatarImage
                        src={`https://avatar.vercel.sh/${lead.ownerId || "team"}.png`}
                      />
                  ...
                  <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <DropdownMenu>
                      ...
                      >
                        ...
                        >
                          <DollarSign className="w-4 h-4 mr-2" /> Send Payment
                          Link
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={(e) => {
                        ...
                        >
                          <CheckCircle2 className="w-4 h-4 mr-2" /> Convert to
                          Client
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
