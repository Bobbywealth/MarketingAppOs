...
import { Users, Megaphone, ArrowUpRight, ArrowDownRight, Activity, Calendar, CheckCircle2, ListTodo, Eye, Plus, Clock, UserPlus } from "lucide-react";
...
export default function ManagerDashboard() {
  ...
  const { role, isManager, isCreatorManager } = usePermissions();

  useEffect(() => {
    if (!(isManager || isCreatorManager)) navigate(getDefaultDashboardPath(role));
  }, [isCreatorManager, isManager, navigate, role]);
  ...
  return (
    <PullToRefresh onRefresh={async () => { await refetch(); }}>
      ...
      <div className="min-h-full gradient-mesh overflow-x-hidden">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs md:text-sm text-muted-foreground/80 mt-0.5">
                Here's your team operations snapshot
              </p>
            </div>
          </div>
        </motion.div>
        ...
        {/* Pipeline by stage (counts only; no $) */}
        {((stats as any)?.pipelineByStage && Object.keys((stats as any).pipelineByStage).length > 0) && (
          <Card className="glass-strong border-0 shadow-xl overflow-hidden">
            <CardHeader className="border-b border-border/50 bg-gradient-to-r from-purple-500/5 via-transparent to-transparent p-4 md:p-6">
              <CardTitle className="text-lg md:text-xl font-semibold">Pipeline (by stage)</CardTitle>
              <p className="text-xs md:text-sm text-muted-foreground mt-1">
                Counts only (no deal value).
              </p>
            </CardHeader>
            <CardContent className="p-4 md:p-6">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {Object.entries((stats as any).pipelineByStage as Record<string, number>).map(([stage, count]) => (
                  <div key={stage} className="rounded-lg border bg-background/60 p-3">
                    <div className="text-xs text-muted-foreground capitalize">{String(stage).replace(/_/g, " ")}</div>
